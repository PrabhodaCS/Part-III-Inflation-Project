...
\usepackage{comment}
\usepackage{natbib}
\usepackage{graphicx}    % already loaded by RevTeX, but shown here for clarity
\usepackage{subfigure}   % RevTeX normally loads this automatically
...
\begin{comment}
    ...
    Inflationary theory was initially articulated in a series of seminal papers by Guth \cite{GuthOriginalPaper}, Linde \cite{LINDE1982389}, Steinhardt \cite{PhysRevLett.48.1220}, and Starobinsky \cite{STAROBINSKY198099}, and serves as a framework that seeks to explicate the observed homogeneity and isotropy of the Universe at present. The theory postulates that in the early Universe, a specific region experienced a phase of rapid exponential expansion, effectively smoothing out all initial inhomogeneities. , a homogeneous and isotropic field permeated spacetime, with its potential energy exceeding its kinetic energy. This condition provided the negative pressure required within the Einstein Field Equations (EFEs) \cite{einstein1915feldgleichungen} for gravity to adopt a repulsive character, facilitating the rapid exponential expansion requisite for inflation. The conclusion of inflation coincides with the breakdown of slow-roll conditions—characterized by minimal field acceleration and kinetic energy compared to potential energy—leading to a reheating process into Standard Model particles \cite{Mukhanov:2005sc}. While inflation demonstrates significant phenomenological success, it lacks a distinct fundamental origin. Most models are constructed on an ad hoc basis and are limited solely by observational data concerning the slope and curvature of the inflaton potential. This paper posits that a particularly satisfactory and dynamic inflationary model can be realized by enforcing local scale invariance (Weyl symmetry \cite{weyl_reine_1918}) in the inflaton-gravity system. Such a symmetry inherently constrains the form of the Lagrangian and establishes connections with broader theoretical principles, such as conformal invariance \cite{francesco_conformal_1997}.
\end{comment}
...
\paragraph*{The Appeal of Scale Invariance} Scale invariance in cosmology seems quite an attractive hypothesis given the nearly scale-invariant spectrum of primordial fluctuations as measured by Planck and WMAP \cite{collaboration_planck_2020, collaboration_bicep_2021}, which might reflect the approximate dilation symmetry of the quasi–de Sitter phase during inflation. We also see that during inflation, which had approximately a de Sitter metric,
\begin{equation} \label{deSitter Conformal}
    \text{d}s^2 = \frac{1}{H^2\eta^2}(\text{d}\eta^2 - \text{d}\Vec{x}^2),
\end{equation} 
remains unchanged for a scale change corresponding to $\Vec{x} \rightarrow \lambda \Vec{x} \,\, \text{and} \,\, \eta \rightarrow \lambda \eta$ (where $\text{d}\eta =  \frac{\text{d}t'}{a(t')}$ is the conformal time), it therefore compels us to consider that any Effective Field Theory (EFT) defined on this background must be scale invariant too, perhaps even locally. This is in line with how EFTs defined on Minkowski space are expected to obey the symmetries of the Lorentz group $SO^+(1,3)$. Much of the Starobinsky potential's success also arises from including the $R^2$ term in the Lagrangian, given $[R^2] = 4$ in natural units, its coefficient is naturally dimensionless and therefore invariant under change of scale $g_{\mu\nu} \rightarrow \Omega^2 g_{\mu\nu}$, for $\Omega$ constant \cite{whitt_fourth-order_1984, Cecchini_2024}.
\begin{comment}
        \wb{
...
The standard Hot Big Bang model successfully describes nucleosynthesis and the CMB. However, there exist some issues with this model, such as the two fine-tuning problems, which are the horizon and the flatness problems (For more details, see \cref{A Primer on Inflation}): 
\begin{comment}
\begin{description}
...
\end{equation}
Given that $\ddot{a} > 0$ and the Friedmann equations (see \cref{Friedmann Eqn 1}), we get $p < -\frac{\rho}{3} \implies w < - \frac{1}{3}$. 
...
\paragraph{Canonical Scalar Field Inflation} 
...
\begin{subequations} \label{presure and density}
\begin{align}   
    \rho &= \frac{1}{2} \dot{\varphi}^2 + V(\varphi) \\
    p &= \frac{1}{2} \dot{\varphi}^2 - V(\varphi)
\end{align}    
\end{subequations}
The third equation, \cref{KGEOM}, is the scalar field equation of motion for a spin-zero particle in the FLRW metric with the potential $V(\varphi)$. 
\begin{comment}
...
\end{equation}
Now that we have all the relevant parameters as functions of the field, we can replace the time derivatives in \cref{KGEOM} with the derivative with respect to the e-folding time N. Using $\text{d}N = H \text{d}t$, we get our final answer of the field evolution as a differential equation with respect to N.
\begin{widetext}
...
Quantum fluctuations of the inflaton field $\varphi$ and the resulting perturbations in the metric generate scalar (curvature) and tensor (gravitational-wave) perturbations, characterized by spectra \cite{baumann2012tasilecturesinflation}
\begin{equation}
...
\end{equation}
Under a local rescaling of the metric, the kinetic term $D_\mu \varphi D_\nu \varphi \rightarrow e^{2\rho}$ while the metric and the volume element transform as indicated above, therefore the combination in the above equation remains invariant, i.e., the action does not change. To remove the explicit mass scale $m$ appearing in the action \cref{25}, we add a compensator scalar field $\phi \rightarrow e^{-\rho}\phi$ (the dilaton) such that the quadratic term remains Weyl-Invariant provided the coupling is dimensionless,
\begin{equation}
    S_\text{Matter} \propto \int_\mathcal{M} \left[\frac{1}{2} D_\mu \varphi D^\mu \varphi - \frac{\mu}{2} \phi^2 \varphi^2 + \frac{1}{2} D_\mu \phi D^\mu \phi \right] ,
\end{equation}
...
\begin{equation}
        S_\text{Gravity} \propto \int_\mathcal{M} \propto ( \beta \varphi^2 + \gamma \phi \varphi +\alpha \phi^2) \mathring{R} .
\end{equation}
...
\begin{aligned}
S &= \int_\mathcal{M} \big[ -\Bigl(\beta\, \varphi^2 + \gamma\, \phi_0\, \varphi + \alpha\, \phi_0^2\Bigr)
...
We now have all the relevant pieces to analyze the model, given the information in \cref{Inflation}  and \cref{Scale Invariant Gravity}. For the starting action \cref{Starting Action}, using a suitable parameter and field redefinition (which we have done with no loss in generality) given by \cref{Variable Dictionary}, we were able to simplify it to one of the form $S_{\text{EH}} + S_{\text{INFLATON}}$. Now, we can analyse the inflaton action by itself,
\begin{equation}
...
\end{align}
Where, in the final potential formula, we have reinstated $\lambda, \kappa$, $\omega$, and $\chi$. To canonicalize the action, that is, to write it in the form with no kinetic coefficient, we will need to redefine the field as 
\begin{equation} \label{40}
    \tilde{\varphi}(\varphi) = \int_0^{\varphi} \sqrt{K(x)} \text{d}x,
\end{equation}
for this, we require $4\alpha \beta > \gamma^2$ to be satisfied, to ensure $K(\varphi) > 0$ everywhere. This is because we need a positive kinetic coefficient to avoid ghost instabilities. Using \cref{KE}, we get,
\begin{equation}
...
\end{equation}
This redefinition also tells us that the original range $\varphi \in (-\infty,\infty)$ gets mapped to $\tilde{\varphi} \in \left( -\sqrt{\frac{12 + k}{2}}M_p \frac{\pi}{2},\sqrt{\frac{12 + k}{2}}M_p \frac{\pi}{2} \right)$. Therefore using this inside the potential \cref{VE}, we get the potential $V(\varphi(\tilde{\varphi}))$, and can write the final canonical inflaton action as
\begin{equation}
...
\begin{align} \label{Weird Approx}
    V \approx V_{\infty}[1 - B'(\tilde{\varphi}_{\text{max}} - \tilde{\varphi}) + \cdots],
\end{align}
...
We observe that this model exhibits behaviour roughly in line with what we would expect from inflationary models. For a more precise analysis, we would need to examine the exact form of the potential \cref{Potential}. This has nine free dimensionless parameters. However, the previous analysis shows us that only certain combinations of parameters affect the observables. To make this exact, we define the new variable $Y$ as:
\begin{align}
...
Optimizing the parameters such that at $N=60$, we get $n_s = 0.9649$, we get the graphs \cref{Optimized Canonical Potential vs Field1}, \cref{Optimized Phase Space potrait1}, and \cref{Optimized Compact Phase Space potrait1} (We have set $M_p = 1$ for these graphs and code):
\begin{figure}[h!]
    \centering
...
%TC:ignore
\appendix
\section{A Primer on Inflaton} \label{A Primer on Inflation}
...
The theory of inflation attempts to resolve the need for extreme fine-tuning of the initial conditions of the universe by positing a period of exponential expansion. As stated in \cref{Inflation}, a decreasing comoving horizon elegantly resolves both these issues.

\subsection{Inflationary Observables}  \label{Inflationary Observables}
Inflation is a robust theory not only because it solves the horizon and flatness problems, but it also makes predictions about the inhomogeneities in the early universe. These primordial density fluctuations are a result of the inherent quantum nature of the inflaton field. 
...
\section{Translating variables} \label{Translating variables}
As explained in the \cref{Introduction}, the papers \cite{Salvio_2022} and \cite{barker2024poincaregaugetheoryconformal} arrive at the same form of the potential, through different theoretical starting points. In the paper \cite{Salvio_2022}, the authors derived an inflaton potential using metric-affine gravity. The motivation here is to give inflation a geometrical explanation. In the paper \cite{barker2024poincaregaugetheoryconformal}, the authors posit a locally scale-invariant action for the inflaton supplemented by the dilaton and the Weyl gauge vector to maintain Weyl Symmetry. 

In \cite{Salvio_2022}, they start with the action, 
\begin{equation}
...
Where $g$ is a dimensionless parameter, this tells us that the model in \cite{barker2024poincaregaugetheoryconformal} has an extra redundant dimensionless parameter that we can absorb into the definitions of the other variables and parameters.
...
\end{equation}
We see this aligns with the canonical scalar field when we set $K = 1$, giving us \cref{KGEOM}. To derive the EOM of the inflaton field with respect to the e-folding time ($N = \int H \text{d}t$), we use the energy density and the pressure of the fluid, which are as follows: 
...
\end{subequations}
\end{widetext}
\paragraph{Slow Roll Inflation for noncanonical scalar field}
...
\end{equation}
For the non-canonical scalar field, the Hamilton-Jacobi equation is: 
...
\begin{equation} \label{3 ricci}
    {}^{(3)}R = \gamma^{ij} \,{}^{(3)}R_{ij}
\end{equation}
\section{Conformal Transformation} \label{Conformal Transformations}

...
In this section, we shall consider a general non-canonical scalar field in the presence of gravity and derive the equations of motion and relevant parameters for such a scalar field. This will provide us with the necessary groundwork for what is to come later in our model (We shall work in the mostly minus convention).
...
\begin{eqnarray}
  ...
  &=& 0. \nonumber
\end{eqnarray}
We see that the equations of motion imply $\alpha$ to be an arbitrary constant, which we can choose to be 1 with no loss in generality. Writing them more suggestively, we get:
\begin{align}
...
\end{align}
However, under a local rescaling, this new curvature scalar still transforms with only Weyl weight 2\footnote{The Weyl weight of a field $\Phi$ is defined as the power of the conformal factor by which it transforms under a local rescaling. It is $\Delta$ if $\Phi \rightarrow e^{-\Delta\rho}\Phi$}, that is, $\mathring{R} \rightarrow e^{-2\rho} \mathring{R}$. This means that, for the gravitational part of our action, we must have a non-minimal coupling of our scalar fields and the Weyl Scalar to give the action Weyl weight 0.

...
\begin{comment}
     We will follow up on the idea and analyze the theory with the inclusion of gravity. We find that the following model is quite interesting and matches quite well with what is expected from an inflationary system.
\end{comment}
\end{abstract}
...
Optimizing the parameters such that at $N=60$, we get $n_s = 0.9649$, we get the below graphs (We have set $M_p = 1$ for these graphs and code):
\begin{figure}[h!]
    \centering
...
\section{CONCLUSION}
We have shown that by demanding local scale invariance in a gravity-scalar theory and analysing the most general action that is linear in curvature, we get a potential that lends itself to be uniquely interpreted as an inflaton potential. 

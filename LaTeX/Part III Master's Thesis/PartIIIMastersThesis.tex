...
\begin{align}
    S = V \int_{t_1}^{t_2} \mathrm{d}t \left[3M_p^2 \left( \frac{- a \dot{a}^2}{\alpha} + \alpha ak\right)+  \frac{ a^3(t)}{2\alpha} \dot{\varphi}^2 \nonumber \\
...
\end{comment}
We see that the equations of motion for $\alpha$ imply it to be an arbitrary constant, which we can choose to be 1 with no loss in generality.
The first two equations \ref{First 2} correspond to the first two Friedmann equations \ref{sometj} if we make the following identification:
\begin{align}   \label{presure and density}
    \rho &= \frac{1}{2} \dot{\varphi}^2 + V(\varphi) \nonumber \\
    p &= \frac{1}{2} \dot{\varphi}^2 - V(\varphi)
\end{align}
(These can be derived from the calculating the energy momentum tensor $T_{\mu\nu} = \frac{2}{\sqrt{-g}} \frac{\delta S}{\delta  g^{\mu \nu}}$).
...
\end{comment}
 To derive the equations of motion of the inflaton field with respect to the e-folding time ($N = \int H \text{d}t$) 
...
\begin{comment}
    or whether the universe is spatially closed, flat, or open ($k \in (-1,0,+1)$)
